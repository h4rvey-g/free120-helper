...
assert.match(html, /<dt>Skipped<\/dt><dd>1<\/dd>/, 'review score summary labels unanswered items as skipped');
assert.match(html, /<option value="omitted">Skipped<\/option>/, 'review status filter labels unanswered items as skipped');
assert.doesNotMatch(html, /<option value="omitted">Omitted<\/option>/, 'review status filter does not show the legacy omitted label');
assert.match(html, /function statusLabel\(status\)/, 'review runtime maps stored omitted statuses to skipped labels');
assert.match(html, /appendDetail\(details, 'Status', statusLabel\(question.status\)\)/, 'review details show status display labels');
